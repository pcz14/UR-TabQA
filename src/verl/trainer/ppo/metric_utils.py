...
def compute_data_metrics(batch: DataProto, use_critic: bool = True, reward_extra_infos_dict: Dict = None, reward_upper_bound=None) -> Dict[str, Any]:
    """
    ...
    for key, value in batch.non_tensor_batch.items():
        ...
        if key in ['reward', 'score']:
            metrics[f"train-reward/{key}/mean"] = sum(value)/len(value)
            metrics[f"train-acc/{key}/ratio"] = sum(np.array(value) == sum(reward_upper_bound.values()))/len(value)
    print('metrics', metrics)
    return metrics

# 测试用例
def compute_group_acc_metrics(
    scores: List,
    index: np.ndarray,
    reward_upper_bound: Dict
):
    ...
    bsz = len(scores)
    for i in range(bsz):
        id2score[index[i]].append(scores[i])  # {'uid1': [1,3,0,0,0,1,0,3]}
    for idx, scores in id2score.items():
        id2all[idx] = (np.array(scores) == sum(reward_upper_bound.values())).all() # {'uid1': 1}
        id2any[idx] = (np.array(scores) == sum(reward_upper_bound.values())).any() # {'uid1': 1}
    metrics = {
        "train-acc/group_all_correct/ratio": np.mean(list(id2all.values())),
        "train-acc/group_any_correct/ratio": np.mean(list(id2any.values())),
        "train-acc/group_all_wrong/ratio": 1 - np.mean(list(id2any.values()))
    }
    print('group_metrics', metrics)
    return metrics
