...
class NaiveRewardManager:
    ...
    def __call__(self, data: DataProto, return_dict=False, step: int=0):
        ...
        for prompt_str, indices in prompt_groups.items():
            ...
            for i in indices:
                ...
                # 如果答案正确，保存代码
                if dict_scores.get('answer', 0) == 3.0:  # 3.0表示答案正确
                    try:
                        # 提取代码
        ...
        for score_info in all_scores:
            if score_info["dict_scores"].get('answer', 0) != 3.0:  # 错误答案
        ...
        # 初始化分项张量
        reward_tensor_items = {
            'format': torch.zeros_like(data.batch['responses'], dtype=torch.float32),
            'execute': torch.zeros_like(data.batch['responses'], dtype=torch.float32),
            'answer': torch.zeros_like(data.batch['responses'], dtype=torch.float32),
            'codebleu': torch.zeros_like(data.batch['responses'], dtype=torch.float32),
            'score': torch.zeros_like(data.batch['responses'], dtype=torch.float32),
            'acc': torch.zeros_like(data.batch['responses'], dtype=torch.float32)
        }
        ...
        for score_info in all_scores:
            ...
            # 设置分项分数
            for key in ['format', 'execute', 'answer', 'codebleu','acc']:
                if key in dict_scores:
                    reward_tensor_items[key][i, valid_response_length - 1] = dict_scores[key]
            # 设置总分到 'score' 键
        ...
        if return_dict:
            return {
                ...
                "reward_extra_info": reward_tensor_items,  # Dict[List[batch_size]]  {'format': [-1, -1, -0.5, ...], 'execute': [...], 'answer': [...]}
            }
        else:
